...
contract Presale is Ownable, ReentrancyGuard {
	...
	mapping(address => uint256) public balanceOfDataGen;
    ...
    *  this method allows to purchase tokens in behalf of another address.*/
    function invest() public payable {
		...
		balanceOfDataGen[msg.sender] = balanceOfDataGen[msg.sender].add(amount.mul(price));
	...
	function getDataGen() public afterClosed nonReentrant {
		require(balanceOfDataGen[msg.sender] > 0, "Zero ETH contributed.");
		uint256 amount = balanceOfDataGen[msg.sender];
		uint256 balance = tokenReward.balanceOf(address(this));
		require(balance >= amount, "Contract has less fund.");
		balanceOfDataGen[msg.sender] = 0;
