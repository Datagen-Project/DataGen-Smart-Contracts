...
contract("VCPrivateSale", accounts => {
    ...
    describe("Claim Datagen and USDC", function() {
        it("has to claim 10% in the first 90 days", async function() {
            ...
            await this.USDCToken.approve(this.contractOpen.address, invested, {from: accounts[4]});
            await this.contractOpen.invest(invested, {from: accounts[4]});
            
            const balanceOfDGBefore00 = await this.contractOpen.checkDataGenFunds(accounts[4]);
            const totalOfDGBefore00 = await this.contractOpen.checkDataGenTotalFunds(accounts[4]);
            const amountRaised0 = await this.contractOpen.amountRaisedDG();
            console.log("dopo -> 1 balance: %s, total %s, amountRaised: %s", balanceOfDGBefore00, totalOfDGBefore00, amountRaised0);
            ...
            const balanceOfDGBefore = await this.contractOpen.checkDataGenFunds(accounts[4]);
            const totalOfDGBefore = await this.contractOpen.checkDataGenTotalFunds(accounts[4]);
            console.log("1 balance: %s, total %s", balanceOfDGBefore, totalOfDGBefore);

            await this.contractOpen.claimDataGen({from: accounts[4]}); 
            await this.contractOpen.claimDataGen({from: accounts[4]}); 
            console.log(maxAmount.toString());

            const balanceOfDGBefore1 = await this.contractOpen.checkDataGenFunds(accounts[4]);
            const totalOfDGBefore1 = await this.contractOpen.checkDataGenTotalFunds(accounts[4]);
            console.log("2 balance: %s, total %s", balanceOfDGBefore1, totalOfDGBefore1);
        ...
        it("has to claim 10% in the first 90 days and 10% of the rest invested amount", async function() {
          ...
          //  await time.increase(time.duration.days(95))
            ...
            balanceOf.toString().should.equal("190000000000000000000000");
        });                          
    });
});