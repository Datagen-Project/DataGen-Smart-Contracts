...
contract ReservedPool is Ownable, ReentrancyGuard {
  ...
  uint256 constant public frAmount = 750000 * (10**18);
  /* First release start time(21/12/2023 00:00:00) */
  uint256 constant public frStart = 1703116800;
  /* Second release tokens */
  uint256 constant public srAmount = 750000 * (10**18);
  /* Second release start time */
  uint256 constant public srStart = 1734220800;
  ...
  function releaseDataGen() external firstRelease nonReentrant {
    ...
    if(block.timestamp < srStart ) {
      ...
      if(transferAmount > 0) {
        ...
        dataGen.safeTransfer(companyWallet, transferAmount);
      }
    }
    else {
      uint256 balance = dataGen.balanceOf(address(this));
      ...
      if( balance > srAmount) {
        dataGen.safeTransfer(companyWallet, (balance.sub(srAmount)));
      }
